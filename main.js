...
searchButton.addEventListener('click', function() {
  ...
  var modelType = select.value;

  if (!isNaN(lat) && !isNaN(lng)) {
    searchNearestPOIs(lat, lng, modelType);
  } else {
    alert('請輸入有效的經緯度');
  }
});
...
// 查詢最近的POI
function searchNearestPOIs(lat, lng, modelType) {
  // 清空現有標記
  markersLayer.clearLayers();

  // 調用後端API獲取最近的POI
  fetch(`http://localhost:3000/nearest-poi?lat=${lat}&lng=${lng}&model_type=${modelType}`)
    ...
    .then(data => {
      ...
      data.forEach(poi => {
        var marker = L.circleMarker([poi.latitude, poi.longitude], {
          color: modelTypeColors[poi.model_type] || 'gray',
          radius: 8
        }).bindPopup(`<b>${poi.name}</b><br>距離：${poi.distance} 米`);
        markersLayer.addLayer(marker);

        infoHtml += `<li>${poi.name}（${poi.model_type}），距離：${poi.distance} 米</li>`;
        displayedTypes.add(poi.model_type);
      });
      infoHtml += `</ul>`;
...
function updateLegend(displayedTypes) {
  ...
  displayedTypes.forEach(type => {
    legendHtml += `<i style="background:${modelTypeColors[type]}; width: 12px; height: 12px; display: inline-block; margin-right: 5px;"></i> ${type}<br>`;
  });
